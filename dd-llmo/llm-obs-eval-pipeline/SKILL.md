...
**Action**: Write a small one-shot publish script to `<output-dir>/_publish_dataset.py`, then execute it. Script content:

```python
"""One-shot dataset publisher for llm-obs-eval-pipeline.

Auto-generated by the pipeline orchestrator — safe to delete after running.
"""
import json
import os
import pathlib
...
def _load_env_files() -> list[str]:
    ...
    Mirrors the discovery order in the pipeline skill's Precheck step 5
    (see dd-llmo/llm-obs-eval-pipeline/SKILL.md). Order: sibling dir of
    this script, app root, cwd, then a parent walk, then
...
RECORDS_PATH = "<absolute path to dataset JSON from Phase 4>"
DATASET_NAME = "<chosen dataset_name from Checkpoint 4>"
...
LLMObs.enable(
    ...
    project_name=PROJECT_NAME,  # project is created lazily here if it does not exist
...
dataset = LLMObs.create_dataset(
    ...
    description=f"Seed dataset for {DATASET_NAME} (pipeline flow, sampled from production traces).",
    records=records,
)
...
- The script's `_load_env_files()` walks the same locations the Precheck already inspected, prints which file(s) loaded keys (never the values), and respects shell-env overrides. If the Precheck already established credentials, the loader will be a no-op.
- `LLMObs.enable(project_name=PROJECT_NAME, ...)` creates the project in Datadog if it does not exist. This is where the `--project-name` you confirmed in the Precheck actually materializes.
- If `_normalize_tags` printed a `WARNING:` line, surface it in Checkpoint 5.
...
- If the script fails with an auth error (401/403), surface the message and stop — do not retry. Tell the user the most likely cause is a stale `.env` value, and that `export DD_API_KEY=... DD_APPLICATION_KEY=...` in the shell takes precedence and can be used to override.
...
3. `Write` for the Phase 5 publish script (`_publish_dataset.py`).
