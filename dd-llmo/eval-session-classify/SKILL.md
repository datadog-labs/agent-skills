...
description: Classify whether a user's intent was satisfied in a Datadog assistant session, using RUM behavioral signals and LLMObs trace data. Produces a satisfaction verdict (yes/partial/no) that feeds into eval-trace-rca for failure analysis. Use when given a session_id to evaluate session quality or generate eval signal.
---

# Skill: eval-session-classify

Given a Datadog assistant session ID, classify whether the user's intent was satisfied.
Uses only the Datadog MCP

---

## Input

A single `session_id` UUID, e.g. `5e707792-9d82-46e5-92aa-bf05b33f5342`.
...
**For `assistant_api` sessions specifically**, you will typically find some of these, but
treat whatever is present as the authoritative set for that span:

| Key pattern | What it classifies |
|-------------|-------------------|
| `tribunal_pod_classifier` | Top-level product area (`rca`, `config`, etc.) |
| `tribunal_config_classifier` | Sub-type within config (`monitor`, `dashboard`, `other`, etc.) |
| `tribunal_intent_classifier` | Intent taxonomy label |
| `tribunal_frustration` | User frustration score |
| `tribunal_laziness` | Response laziness score |
| `tribunal_groundedness` | Whether response was grounded in tool results |
...
Run two RUM queries in parallel via `aggregate_rum_events` (SQL-based, `rum` toolset).
...
**What Query 2 captures:**
- `command-assistant.panel.open/close`
- `command-assistant.entity.add/remove`
- `command-assistant.message.send`
- `command-assistant.message.feedback` (thumbs up/down)
- `command-assistant.suggestion.select`
- `command-assistant.thinking.toggled` / `click on Reasoning`
- `command-assistant.tool_call.*`
- `workbench.dock.show`
- `ai-experiences.chat-submit` (message sends)
- `ai-experiences.chat-submit-feedback` (feedback submit)
- `click on Bad response` / `click on Incorrect result` / `click on Submit`
- `Rendered a Code block`
...
**External customer RUM noise**: External customer sessions (org_id ≠ 2) generate far more
framework telemetry per page load. APM service pages can emit 200+ custom events in 2 minutes,
exhausting the LIMIT before reaching the session time. When this happens, add a name-prefix
filter to query B to target only assistant-specific actions:

```python
aggregate_rum_events(
  event_type    = "action",
  filter        = "@action.type:custom @usr.email:<user_handle> @action.name:(command-assistant* OR ai-experiences* OR workbench* OR navbar.topbar.ask-bits*)",
  from          = <broader window — e.g. same day>,
  to            = <post_end>,
...
This typically reduces thousands of noise events to the 10–20 assistant-specific signal events.

**To interpret what the action names mean for your agent, consult the agent-specific RUM
action reference file.** For `assistant_api` (Bits AI / CMD+I), see:
...
From the agent loop: which tools were called (with arguments), what docs/data was retrieved,
what the final response said.
...
- Content of external customer sessions (org_id ≠ 2): fully masked — see error codes above.
